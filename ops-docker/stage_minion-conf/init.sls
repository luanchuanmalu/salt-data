...
openvswitch-db:
  cmd.run:
    - unless: semanage fcontext -l | grep '/etc/openvswitch'
    - name: 'sh openvswitchdb.sh'
    - cwd: /srv/salt-data/minion-packages/openvswitch
    - require:
      - pkg: openvswitch
      - file: /srv/salt-data/minion-packages/openvswitch/openvswitchdb.sh
...
/srv/salt-data/minion-conf/openvswitch:
  file.directory:
    ...
    - group: root
    - makedirs: True
#Stage different with prod, source different
/srv/salt-data/minion-conf/minionlist.ini:
  file:
    - managed
    - source: salt://stage_minion-conf/minionlist.ini
    - require:
...
/etc/nginx/nginx.conf:
  file:
    - managed
    - source: salt://stage_minion-conf/nginx.conf
#openswitch bridge
openvswitch-bridge:
  cmd.run:
